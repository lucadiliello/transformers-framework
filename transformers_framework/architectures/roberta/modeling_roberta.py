...
class RobertaExtendedClassificationHead(nn.Module):
    ...
    def __init__(self, config: RobertaExtendedConfig):
        super().__init__()
        self.config = config
        self.classifier = ClassificationHead(config, num_labels=config.k * config.num_labels)
    ...
    def forward(self, hidden_state: torch.Tensor) -> torch.Tensor:
        ...
        hidden_state = hidden_state.mean(dim=1) if self.config.aggregate_hidden_states else hidden_state[:, 0, :]
        logits = self.classifier(hidden_state)
        logits = logits.view(-1, self.config.k, self.config.num_labels)
        return logits
